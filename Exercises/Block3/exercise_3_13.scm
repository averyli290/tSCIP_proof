...
(define (new-apply proc vals) ; Evaluates proc in a new frame with variables bound
...
(define (install-eval-package)
    ...
    (set-table '+ (lambda (expr env) (+ (new-eval (cadr expr) env) (new-eval (caddr expr) env))))
    (set-table '/ (lambda (expr env) (/ (new-eval (cadr expr) env) (new-eval (caddr expr) env))))
    (set-table '- (lambda (expr env) (- (new-eval (cadr expr) env) (new-eval (caddr expr) env))))
    (set-table '* (lambda (expr env) (* (new-eval (cadr expr) env) (new-eval (caddr expr) env))))
    (set-table '= (lambda (expr env) (if (= (new-eval (cadr expr) env) (new-eval (caddr expr) env)) 'true-object 'false-object)))
