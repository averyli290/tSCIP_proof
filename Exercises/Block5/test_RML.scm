...
(define (the-program) ; the lisp 2 program that will be loaded into the special machine
    '(
      ;(define x 1)
      ;(define test-l (lambda (x) (if (= x 2) 0 1)))
      ;(test-l 1)
      ;(+ 1 1)
...
(define (get-RML)
  '(
    ...
    eval-loop
        ...
        (pop (reg env))
        (push (reg env)) ; pushing the environment to get it back when done (goign back to global when done)
    ...
    ev-apply-2 ; the next stage of apply starts with the evaluated procedure in (reg proc) and setting up for the evaluating the arguments
        ...
        (pop (reg exp))
        (assign (reg proc) (reg val)) ; assigned proc to result of evaluating CAR (reg exp)
