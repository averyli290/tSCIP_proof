...
(define (install-rml-eval-package)
    ...
    (define (get-label-number label-pair) (cdr label-pair)) ; gets the label number in a ('label #) pair

    (define (label-helper line-number instructions) ; gets the instructions at n lines in
        (if (= 3 line-number)
            instructions
            (label-helper (- 1 line-number) (cdr instructions))))

    ; functions that need instructions
    (set-table 'goto (lambda (instructions expr) (newline) (display "Here:: ") (label-helper (get-label-number (get-var-value (cadar (get-contents expr)))) instructions) )) ; need to return ('label (get-var-value label)) because the instructions iteration needs to run (get-var-value label) and it uses cdr 
...
(define (make-machine reg-names op-list instructions)
    (define (setup) ; adds all registers to the frame and operations to the table
        ...
        (define (iter-label code line-number)
            (if (null? code)
                'dummy
                (if (symbol? (car code)) ; tests to see if there is a label
                    (begin (define-var (car code) (cons 'label line-number)) (iter-label (cdr code) (+ 1 line-number))) ; adds the label-code pair to the table
                    (iter-label (cdr code) (+ 1 line-number)))))

        (begin (iter-reg reg-names) (iter-op op-list) (iter-label instructions 1)))
...
;(get-register-contents gcd-machine 'test-label)
frame
